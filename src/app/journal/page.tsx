...
export default function JournalPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-violet-950 to-slate-950 text-white flex flex-col">
      <Header />
      <main className="flex-1 max-w-5xl mx-auto px-6 pt-24 pb-14">
        ...
        <section className="space-y-4">
          {entries.length === 0 ? (
            ...
            entries.map((entry) => (
              <div key={entry._id ?? entry.title} className="rounded-3xl border border-purple-700/30 bg-slate-950/80 p-6 shadow-lg shadow-black/20">
                ...
                <p className="text-slate-200 whitespace-pre-line">{entry.content}</p>
              </div>
            ))
          )}
        </section>
